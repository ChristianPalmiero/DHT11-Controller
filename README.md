...
* MUXES: it describes a cascade of multiplexers that, according to the value of the first three input switches (SW0, SW1, SW2), outputs the corresponding 4-bit nibble of the data coming from the SIPO to display.
* CHECKSUM_CONTROLLER: it describes a combinational block that takes as input the data coming from the SIPO, computes the checksum and compares it with the one that has been computed and sent by the sensor.
* MUX: it describes a multiplexer that, according to the value of SW3, either displays the 4-bit nibble output by the MUXES process (check state) or gathers together and shows some error information coming from the control unit (error state). 
