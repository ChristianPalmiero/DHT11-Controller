...
* **PULSE_GEN**: it is a multiple-stage pulse generator that detects a logic value change on the "data_in" line and generates a 1-clock cycle pulse on the falling output if there is a falling-edge and one pulse of the same duration on the rising output if there is a rising-edge.
